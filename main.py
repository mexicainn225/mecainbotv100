...
@app.route('/')
def home():
    return "Système Multi-Jeux Pro - Connecté"
...
def get_base_min(game):
    ...
    if conf and 'minute' in conf:
        return conf['minute']
    return 0 # Par défaut si non configuré
...
def get_prediction(game_type):
    ...
    intervalle = 21 if game_type == "LUCKY" else 13 # Ligne pour changer la minute Aviator (13)
    ...
    if game_type == "LUCKY":
        ...
        # Réglages Aviator : Objectif min 10X, Sécurité min 4X
        cote = round(random.uniform(10.0, 85.0), 2)
...
def game_handler(msg):
    ...
    if msg.from_user.id == ADMIN_ID or u.get('is_vip'):
        ...
        bot.send_message(msg.chat.id, f"⚠️ **ACCÈS VIP REQUIS**\n\nInscrivez-vous avec le code `{CODE_PROMO}` et envoyez votre ID ici.")
...
def stats_handler(msg):
    bot.send_message(msg.chat.id, "📊 **PERFORMANCES**\n🚀 Lucky Jet : `98.4%` \n✈️ Aviator : `97.8%` \n🛰 Serveur : `Stable`", parse_mode='Markdown')

@bot.message_handler(func=lambda m: m.text.isdigit() and len(m.text) >= 7)
def handle_id(msg):
    kb = telebot.types.InlineKeyboardMarkup().add(
        telebot.types.InlineKeyboardButton("✅ VALIDER", callback_data=f"val_{msg.from_user.id}"),
