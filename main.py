...
def get_grosse_cote_signal():
    ...
    target_time = now.replace(minute=target_min, second=0, microsecond=0)
    random.seed(target_time.timestamp())
...
def big_sig(msg):
    ...
    if msg.from_user.id == ADMIN_ID or u.get('is_vip'):
        ...
        if res is None or res == "EXPIRED":
            bot.send_message(msg.chat.id, "⏳ Aucun créneau disponible actuellement.")
            return
...
def config_grosse(msg):
    ...
    bot.send_message(ADMIN_ID, "Entrez les minutes (ex: 12, 45) :")
...
def save_grosse(msg):
    try:
        ...
        bot.send_message(ADMIN_ID, "Enregistré.")
        notify_all_vips("🔔 **INFORMATION**\n\nNouveaux signaux disponibles dans la section **💎 GROSSE CÔTE**.")
    except:
