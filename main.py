...
@app.route('/')
def home():
    return "Système Mexicain225 - Protection Totale Active 🚀"
...
def get_next_single_signal():
    ...
    cote = round(random.uniform(5.0, 120.0), 2)
    prev = round(random.uniform(5.0, 10.0), 2)
    random.seed()
...
def send_subscription_block(chat_id):
    txt_vip = (f"⚠️ **ACCÈS PRIVÉ REQUIS**\n\n"
               ...
               f"🚀 *Une fois activé, vous aurez accès à vie.*")
...
def start(msg):
    ...
    markup.add(*btns)
    bot.send_message(msg.chat.id, "👋 Bienvenue ! Choisissez une option.", reply_markup=markup)

@bot.message_handler(func=lambda m: m.text == "🚀 OBTENIR UN SIGNAL")
def handle_signal_normal(msg):
    ...
    if u_id == ADMIN_ID or user_data.get('is_vip'):
        ...
        time_range = f"{t_time.strftime('%H:%M')} à {(t_time + timedelta(minutes=1)).strftime('%H:%M')}"
...
def config_grosse(msg):
    ...
    bot.send_message(ADMIN_ID, "🔥 Minute cible pour l'heure suivante (Rattrapage) :")
...
def save_configs(msg):
    if msg.text.isdigit():
        ...
        if admin_state[ADMIN_ID] == "WAITING_MIN":
            ...
            bot.send_message(ADMIN_ID, f"✅ Grosse Côte programmée ({val-10} et {val}).")
...
def handle_id(msg):
    ...
    bot.send_message(msg.chat.id, "✅ ID reçu ! Activation en cours par nos serveurs.")

@bot.callback_query_handler(func=lambda c: c.data.startswith("val_"))
def val_callback(c):
    uid = int(c.data.split("_")[1])
    users_col.update_one({"_id": uid}, {"$set": {"is_vip": True}}, upsert=True)
    bot.send_message(uid, "🌟 **FÉLICITATIONS ! Ton accès est désormais activé.**\n\nTu peux utiliser tous les boutons du robot.")
